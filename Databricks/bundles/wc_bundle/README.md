...
This bundle has a job that will trigger the pipeline run when the `main.wc.wc_teams_bronze` table that was created through the [:star:`scalable_ingestion`](/Databricks/terraform/scalable_ingestion/) is updated.
...
```bash
cd Databricks/bundles/wc_bundle
